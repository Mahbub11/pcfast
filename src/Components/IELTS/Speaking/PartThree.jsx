...
export default function PartThree({ partThreeQuestion, handleSetion }) {
  ...
  const [partThreeUserSet, setPartThreeUserSet] = useState([]);
  ...
  const qLength = Object.keys(partThreeQuestion).length - 1;
  ...
  const startEv = async () => {
    await axiosInstance
      .post(`${API_LEVEL}/ielts/ev/stat`, { message: partThreeUserSet })
      .then(async (response) => {
        const data = {
          3: response.data.stat,
        };
  ...
  const getCompletion = async () => {
    await axiosInstance
      .post(`${API_LEVEL}/ielts/ev/completion`, { message: partThreeUserSet })
      .then((response) => {
        const data = {
          3: response.data.errorList,
        };

        dispatch(saveIeltsSpeakingAssByPart(data));

        setLoadings(false);
        setNextPartBtnState(false);
        handleSetion();
      });
  };
  ...
  const decodeAudio = async (audioBlobUrl) => {
    await sendToWhisper(audioBlobUrl)
      .then((res) => {
        setPartThreeUserSet((prev) => [
          ...prev,
          {
            question: partThreeQuestion[currentQuestion].text,
  ...
  // useEffect(() => {
  //   const audio = partOneQuestion[`${currentQuestionNumber}`].audio;
  //   // load(audio);
  // }, [recordingBlob, currentQuestionNumber,load]);
  ...
  const listenCurrentQuestion = (val) => {
    if (isRecording) {
      stopRecording(); // Stop recording if it is currently in progress
    }
    startTimer();
    setPlayBtnState(false);
    const audio = partThreeQuestion[val].audio;
    ...
    <div>
      ...
      >
        <div className="self-center">
          <h1 className="font-[700] text-[20px] font-poppinsBold">PART 3</h1>
          <h1>Listen Carefully and record your Answer.</h1>
      ...
      >
        <Button
          ...
          className="mt-3 bg-tahiti border-none
