...
import { Select, Space, Input, Modal, Statistic, notification } from "antd";
...
export default function ContainerPageSAL() {
  ...
  const [notificationS, setNState] = useState(false);
  const [index, setIndex] = useState();
  const [show, isShow] = useState(false);
  const [tableData, setTableData] = useState(listSAL);
  const [filterData, setFilterData] = useState(listSAL);
    ...
    <div className="relative">
      <div>
        ...
        <div className=" h-[20rem] w-full mt-10">
          <ListData
            list={filterData.length ===0 ? listSAL :filterData}
            title="Read Aloud"
            type="rc-r"
            handleQ={handleQ}
          ></ListData>
