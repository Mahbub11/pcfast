...
export default function ListDataIELTS({ type, list, title, handleQ }) {
        ...
        <div>
          {
            ...
            <a  className="sm:text-[15px] md:text-[21px] font-montserrat font-[400] flex gap-2"
             href={`${LIVE_URL}ielts/practice/${type}/${record.index}`}>
              <p>{record.index}.</p>
              <p>{title}</p>
            </a>
  ...
  return (
    <div className="m-aut flex flex-col justify-center sm:pb-5">
      <div className="sm:w-full md:w-[90%] m-auto text-[21px] sm:overflow-scroll md:overflow-hidden">
        <Table
          locale={{ emptyText: <span>Loading....</span> }}
          dataSource={list}
          columns={columns}
